...
function stopGroup(signal, reason) {
  if (!terminationReason && reason !== 'DIRECT_CHILD_CLOSED_GROUP_CLEANUP') {
    terminationReason = reason;
  }
  if (child?.pid) {
    try { process.kill(-child.pid, signal); } catch (error) {
      if (error.code !== 'ESRCH') {
        containmentError = `GROUP_SIGNAL_ERROR:${error.code ?? 'UNKNOWN'}:${error.message}`;
        process.stderr.write(`${containmentError}\n`);
      }
    }
  }
}
...
child.on('close', (code, signal) => {
  terminal = true;
  stopGroup('SIGKILL', 'DIRECT_CHILD_CLOSED_GROUP_CLEANUP');
  ...
  const state = containmentError ? 'FAILED_CONTAINMENT'
    : terminationReason === 'SCIENTIFIC_DEADLINE_EXCEEDED'
