...
def solve_sparse_v10(system,u,max_iterations=18,tolerance=1e-8):
    ...
    for iteration in range(max_iterations):
        ...
        step=lsqr(jac,-base["scaled"],atol=1e-12,btol=1e-12,
                  iter_lim=4*system.dimension)[0]
...
def qualification_gate(levels, independent_final, *, tolerance=1e-8):
    """Require any later pair of successive refinements and a final second root."""
    qualifying_pair = None
    for first, second in zip(levels[:-1], levels[1:]):
        if all(row["numericalAccepted"] and row["reducedJacobian"]["fullRank"] and
               row["reducedJacobian"]["rank"] == row["reducedJacobian"]["dimension"]
               for row in (first, second)) and all(
            second[key] <= tolerance for key in (
                "maximumScaledFluxDriftFromPrevious",
                "maximumInterfaceDriftFromPrevious",
                "maximumCommonCoordinateProfileDriftFromPrevious",
            )
        ):
            qualifying_pair = [first["nodes"], second["nodes"]]
    independent_ok = bool(independent_final["numericalAccepted"] and
        independent_final["reducedJacobian"]["fullRank"] and
        independent_final["maximumScaledFluxDifference"] <= tolerance and
        independent_final["maximumInterfaceDifference"] <= tolerance and
        independent_final["maximumCommonCoordinateProfileDifference"] <= tolerance)
    return {"successiveQualifyingPair": qualifying_pair,
            "independentFinalQualified": independent_ok,
            "qualified": bool(qualifying_pair is not None and independent_ok)}