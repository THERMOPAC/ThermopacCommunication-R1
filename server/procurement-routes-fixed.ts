...
export function setupProcurementRoutes(app: Router) {
  ...
  app.get('/api/procurement/purchase-orders/preview/:projectId', ensureAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // Use a raw SQL query to get master items with explicit column aliases
      const { rows: masterItemsList } = await db.execute(
        sql`SELECT 
            id, 
            item_code as "itemCode", 
            description, 
            make_or_buy as "makeOrBuy", 
            unit, 
            preferred_vendor_id as "preferredVendorId", 
            estimated_cost as "estimatedCost" 
          FROM master_items 
          WHERE id IN (${masterItemIds.join(',')})`
      );
