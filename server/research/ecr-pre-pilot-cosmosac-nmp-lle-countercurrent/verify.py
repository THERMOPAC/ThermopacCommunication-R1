...
for case in data["temperatureCases"]:
    ...
    for trial in case["trials"]:
        n = trial["stageCount"]
        check(f"{case['temperatureC']:.0f}C NT{n} overall balance", trial["maximumOverallComponentBalanceResidualMol"] <= 1e-8)
        ...
        for stage in trial["stages"]:
            ...
            check(
                f"{case['temperatureC']:.0f}C NT{n} stage {stage['stageFromFeedEnd']} equilibrium",
                float(np.max(np.abs(mu))) <= 2e-5,
            )
