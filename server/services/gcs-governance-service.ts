...
const SEED_RULES = [
  ...
  { moduleKey: 'epc', submoduleKey: 'ecn',          documentType: 'ECN',             displayName: 'Engineering Change Notice',   rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/{NNN}/ECN/{DocNumber}/rev-{rev}/{filename}',                      revisionMode: 'numeric',   notes: 'Added 2026-05. Separate lifecycle stage from ECR — ECN is the approved change notice issued after ECR is processed. drawing-ecr-ecn-routes.ts' },
  ...
  { moduleKey: 'epc', submoduleKey: 'quotation',     documentType: 'QUOTATION',     displayName: 'Offer PDF (Standalone)',              rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/Open_Quotations/{OfferNo}/rev-{rev}/{Seq}-{Label}.pdf', revisionMode: 'numeric',   notes: 'buildQuotationGcsPath() in epc-coding.ts. Unified Open_Quotations root (2026-05): both standalone and project-linked offers share the same path structure. Revisions increment until offer is confirmed/frozen.' },
  { moduleKey: 'epc', submoduleKey: 'epc_quotation', documentType: 'EPC_QUOTATION', displayName: 'Offer PDF (Project-Linked)',           rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/Open_Quotations/{OfferNo}/rev-{rev}/{Seq}-{Label}.pdf', revisionMode: 'numeric',   notes: 'buildEpcQtnGcsPath() in epc-coding.ts. Unified Open_Quotations root (2026-05): same path structure as standalone QUOTATION. rev-na removed; real revision used. Stores to epc_document_attachments.' },
  // DVS
  ...
  { moduleKey: 'qms', submoduleKey: 'wpqr',         documentType: 'WPQR',            displayName: 'Welder Performance Qualification Record', rootPrefix: 'QMS', pathTemplate: 'QMS/WPQR/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}',                           revisionMode: 'numeric',   notes: 'TRANSITIONAL ROOT — Family B (company-level). wpqr-routes.ts via qms-file-governance.ts. WPQR = Welder Performance Qualification Record — distinct from WPS/PQR (QMS/WPS/). Family B target: TPEL/QMS/{CompanyFY}/WPQR/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}' },
  { moduleKey: 'qms', submoduleKey: 'pma',          documentType: 'PMA',             displayName: 'Particular Material Appraisal', rootPrefix: 'QMS', pathTemplate: 'QMS/PMA/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}',                                      revisionMode: 'numeric',   notes: 'TRANSITIONAL ROOT — Family B (company-level). pma-routes.ts via qms-file-governance.ts. PMA identified by PMA-YYYY-XXX, not project-specific. Family B target: TPEL/QMS/{CompanyFY}/PMA/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}' },
  { moduleKey: 'qms', submoduleKey: 'calibration',  documentType: 'CALIBRATION_CERT',displayName: 'Calibration Certificate',     rootPrefix: 'QMS', pathTemplate: 'QMS/Calibration/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}',                                   revisionMode: 'numeric',   notes: 'CORRECTED 2026-05 — flat-file template QMS/Instrument/{filename} matched deprecated uploadCalibrationCertificate() utility only. Phase 2B standard: createRevision(module=Calibration) → QMS/Calibration/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}. Legacy flat files at QMS/Instrument/{INST-XXXXX}.pdf are retained in GCS as orphans (no deletion). TRANSITIONAL ROOT — Family B (company-level). Family B target: TPEL/QMS/{CompanyFY}/CALIBRATION/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}' },
  ...
  { moduleKey: 'qms', submoduleKey: 'welder_cert',  documentType: 'WELDER_CERT',     displayName: 'Welder Qualification Certificate', rootPrefix: 'QMS', pathTemplate: 'QMS/WelderManagement/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}',               revisionMode: 'numeric',   notes: 'CORRECTED 2026-05 — template was erroneously QMS/WelderCertificates/; actual path from createRevision(module=WelderManagement) is QMS/WelderManagement/. No files moved. TRANSITIONAL ROOT — Family B (company-level). welder-certificate-routes.ts via qms-file-governance.ts. Linked to welder personnel, not projects. Family B target: TPEL/QMS/{CompanyFY}/WELDER-CERTS/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}' },
  ...
  { moduleKey: 'qms', submoduleKey: 'ncr',          documentType: 'NCR',             displayName: 'Non-Conformance Report',      rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/{NNN}/QMS/NCR/{NcrNumber}/rev-{rev}/{filename}',                    revisionMode: 'numeric',   notes: 'Added 2026-05. Family A — project-specific. Raised by QA team against EPC project deliverables. NcrNumber format: NCR-YYYY-NNN. Correct TPEL root from addition.' },
  { moduleKey: 'qms', submoduleKey: 'test_proc',    documentType: 'TEST_PROCEDURE',  displayName: 'Test Procedure Document',     rootPrefix: 'QMS', pathTemplate: 'QMS/TestProcedures/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}',                             revisionMode: 'numeric',   notes: 'TRANSITIONAL ROOT — Family B (company-level). test-procedures-routes.ts via qms-file-governance.ts. Company-approved NDT procedures used across projects. Family B target: TPEL/QMS/{CompanyFY}/TEST-PROCEDURES/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}' },
  { moduleKey: 'qms', submoduleKey: 'wps_pqr',      documentType: 'WPS_PQR',         displayName: 'Welding Procedure Spec / PQR',rootPrefix: 'QMS', pathTemplate: 'QMS/WPS/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}',                                        revisionMode: 'numeric',   notes: 'TRANSITIONAL ROOT — Family B (company-level). wps-pqr-routes.ts. WPS/PQR under QMS/WPS/ (distinct from QMS/WPQR/ = Welder Performance Qualification). Corrected 2026-05: was incorrectly sharing QMS/WPQR/ path. Family B target: TPEL/QMS/{CompanyFY}/WPS-PQR/{DocNumber}/rev-{rev}/{Seq}-{Label}.{ext}' },
