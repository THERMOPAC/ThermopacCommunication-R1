...
const SEED_TOKENS = [
  ...
  // HR module tokens — added with HR governance structure 2026-06
  { tokenName: 'CandidateID',  description: 'Candidate identifier for recruitment', exampleValue: 'CAND-0042',    sourceDescription: 'System-assigned candidate ID from recruitment record. Format: CAND-NNNN' },
  { tokenName: 'CandidateName',description: 'Candidate name slug',                  exampleValue: 'Rajesh_Kumar', sourceDescription: 'Sanitised candidate full name from recruitment record. Spaces → underscore.' },
];
...
const SEED_RULES = [
  ...
  { moduleKey: 'epc', submoduleKey: 'co',           documentType: 'CO_DOCUMENT',     displayName: 'Customer Order Document',     rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/SOR_{Code}/1_Sales/3_Order_Contract/{Seq}-{Label}-rev-{rev}.pdf', revisionMode: 'numeric',   notes: 'Initial CO/PO snapshot auto-saved at end of offer conversion via storeConfirmationDocToGcs(). Post-conversion uploads via customer-order-document-routes.ts. {Code}=EPC project code (e.g. 2627-018). {Seq}=001,{Label}=purchase-order,{rev}=00 for conversion snapshot.' },
  ...
  { moduleKey: 'epc', submoduleKey: 'procurement',  documentType: 'DATASHEET',       displayName: 'PPPC Procurement Datasheet (Legacy)',  rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/{NNN}/PROCUREMENT/DATASHEETS/{ListNo}/{Tag}/{Seq}_ds-rev-{rev}.{ext}', revisionMode: 'numeric', notes: 'SUPERSEDED by PPPC_DATASHEET — kept for GCS rule audit continuity only. pppc-routes.ts now uses PPPC_DATASHEET.' },
  { moduleKey: 'epc', submoduleKey: 'pppc_datasheet', documentType: 'PPPC_DATASHEET', displayName: 'PPPC Buy List Datasheet', rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/{NNN}/PROCUREMENT/DATASHEETS/{ListNo}/{Tag}/{Seq}_ds-rev-{rev}.{ext}', revisionMode: 'numeric', notes: 'pppc-routes.ts — buy list line selection datasheet upload. Governed via resolveGcsPath(). Tokens: CC, CO, Cust, FY, NNN, ListNo, Tag, Seq (lineId), rev (revisionSeq), ext. Phase 3 compliance 2026-06.' },
  ...
  { moduleKey: 'epc', submoduleKey: 'final_offer',   documentType: 'FINAL_OFFER',   displayName: 'Final Locked Offer PDF',             rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/SOR_{Code}/1_Sales/2_Final_Offer/{OfferNo}/{Seq}-{Label}-rev-{rev}.pdf', revisionMode: 'numeric',   notes: 'Immutable snapshot auto-saved at the END of offer conversion (after project code is confirmed, execution drafts, and full-auto pipeline). offer-conversion.ts via storeFinalOfferPdfToGcs(). {Code}=EPC project code (e.g. 2627-019). {OfferNo}=offer number with slashes replaced by dashes. {Seq}=001. {Label}=final-offer.' },
  // DVS
  ...
  // Admin — target paths set per Governance Clarification Plan 2026-05 (Option C).
  // {CompanyFY} = April-March FY e.g. 2526. Existing files remain at old roots until migration.
  { moduleKey: 'admin', submoduleKey: 'business_trip',documentType: 'TRIP_DOCUMENT',  displayName: 'Business Trip Document',       rootPrefix: 'TPEL/ADMIN/TRIPS', pathTemplate: 'TPEL/ADMIN/TRIPS/{CompanyFY}/{EmployeeName}/{Destination}/{DocType}/{filename}', revisionMode: 'none',       notes: 'Family B — company admin. trip-management-routes.ts. Canonical path: TPEL/ADMIN/TRIPS/{CompanyFY}/... (TRIPS before FY). Migration completed 2026-06-15.' },
  { moduleKey: 'admin', submoduleKey: 'visa',         documentType: 'VISA_DOCUMENT',   displayName: 'Visa / Travel Document',       rootPrefix: 'TPEL/ADMIN/VISA',  pathTemplate: 'TPEL/ADMIN/VISA/{CompanyFY}/{EmployeeName}/{Category}/{filename}',              revisionMode: 'none',       notes: 'Family B — company admin. visa-management-routes.ts. Canonical path: TPEL/ADMIN/VISA/{CompanyFY}/... (VISA before FY). Migration completed 2026-06-16.' },
  // HR — Employee lifecycle governance 2026-06.
  // Path convention: TPEL/HR/{EmployeeCode}_{EmployeeName}/... for employee-scoped docs.
  // Recruitment uses CandidateID_{CandidateName} (pre-hire, no EmployeeCode yet).
  // {DocType} is the discriminator within each submodule (e.g. OFFER_LETTER, SALARY_SLIP).
  // {CompanyFY} added for time-periodic submodules (Compensation, Performance).
  { moduleKey: 'hr', submoduleKey: 'recruitment',        documentType: 'HR_RECRUITMENT',    displayName: 'HR Recruitment Document',          rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/RECRUITMENT/{CandidateID}_{CandidateName}/{DocType}/{filename}',                            revisionMode: 'none', notes: 'Pre-hire documents. No EmployeeCode yet — scoped by CandidateID_{CandidateName}. DocType: APPLICATION, RESUME, INTERVIEW_ASSESSMENT, OFFER_LETTER, BGV_REPORT.' },
  { moduleKey: 'hr', submoduleKey: 'employee_master',    documentType: 'HR_EMPLOYEE_MASTER',displayName: 'HR Employee Master Document',       rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/MASTER/{DocType}/{filename}',                                revisionMode: 'none', notes: 'Employee anchor record. DocType: EMPLOYEE_PROFILE.' },
  { moduleKey: 'hr', submoduleKey: 'onboarding',         documentType: 'HR_ONBOARDING',     displayName: 'HR Onboarding Document',           rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/ONBOARDING/{DocType}/{filename}',                              revisionMode: 'none', notes: 'Onboarding documents. DocType: JOINING_FORM, APPOINTMENT_LETTER, NDA.' },
  { moduleKey: 'hr', submoduleKey: 'identity_compliance',documentType: 'HR_IDENTITY',       displayName: 'HR Identity & Compliance Document', rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/IDENTITY/{DocType}/{filename}',                               revisionMode: 'none', notes: 'PII-sensitive — restrict signed URL access to HR/Superuser roles at route level. DocType: AADHAAR, PAN, BANK_DETAILS, PF_DOC, ESIC_DOC, NOMINATION_FORM.' },
  { moduleKey: 'hr', submoduleKey: 'qualifications',     documentType: 'HR_QUALIFICATION',  displayName: 'HR Qualification Document',         rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/QUALIFICATIONS/{DocType}/{filename}',                        revisionMode: 'none', notes: 'Academic and professional credentials. DocType: EDU_CERT, EXP_CERT, PROFESSIONAL_CERT, TRAINING_CERT.' },
  { moduleKey: 'hr', submoduleKey: 'training',           documentType: 'HR_TRAINING',       displayName: 'HR Training Record',               rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/TRAINING/{DocType}/{filename}',                                revisionMode: 'none', notes: 'Training and competency records. DocType: TRAINING_RECORD, CERTIFICATION, COMPETENCY_ASSESSMENT, INDUCTION.' },
  { moduleKey: 'hr', submoduleKey: 'employment_records', documentType: 'HR_EMPLOYMENT',     displayName: 'HR Employment Record',             rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/EMPLOYMENT/{DocType}/{filename}',                             revisionMode: 'none', notes: 'Career event letters. DocType: CONFIRMATION_LETTER, PROMOTION_LETTER, TRANSFER_LETTER, ROLE_CHANGE, DEPT_CHANGE.' },
  { moduleKey: 'hr', submoduleKey: 'compensation',       documentType: 'HR_COMPENSATION',   displayName: 'HR Compensation & Payroll Document',rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/COMPENSATION/{CompanyFY}/{DocType}/{filename}',                revisionMode: 'none', notes: 'Period-bound financial documents. {CompanyFY}=April-March FY (e.g. 2526). DocType: INCREMENT_LETTER, BONUS_LETTER, SALARY_SLIP, FORM_16.' },
  { moduleKey: 'hr', submoduleKey: 'performance',        documentType: 'HR_PERFORMANCE',    displayName: 'HR Performance & Appraisal Document',rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/PERFORMANCE/{CompanyFY}/{DocType}/{filename}',               revisionMode: 'none', notes: 'Period-bound appraisal records. {CompanyFY}=April-March FY. DocType: APPRAISAL. Governs all appraisal PDFs going forward; historical migration approved but separate.' },
  { moduleKey: 'hr', submoduleKey: 'disciplinary',       documentType: 'HR_DISCIPLINARY',   displayName: 'HR Disciplinary Record',           rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/DISCIPLINARY/{DocType}/{filename}',                           revisionMode: 'none', notes: 'Sensitive disciplinary event records. DocType: WARNING_LETTER, SHOW_CAUSE, INVESTIGATION_REPORT, GRIEVANCE, LEGAL_HR.' },
  { moduleKey: 'hr', submoduleKey: 'medical',            documentType: 'HR_MEDICAL',        displayName: 'HR Medical Record',                rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/MEDICAL/{DocType}/{filename}',                                  revisionMode: 'none', notes: 'Employee health and medical documents. DocType: MEDICAL_CERT, FITNESS_CERT, HEALTH_RECORD, INSURANCE_DOC.' },
  { moduleKey: 'hr', submoduleKey: 'separation',         documentType: 'HR_SEPARATION',     displayName: 'HR Separation Document',           rootPrefix: 'TPEL/HR', pathTemplate: 'TPEL/HR/{EmployeeCode}_{EmployeeName}/SEPARATION/{DocType}/{filename}',                              revisionMode: 'none', notes: 'Terminal lifecycle documents. Employee path TPEL/HR/{EmployeeCode}_{EmployeeName}/ is retained permanently after separation — no archival move. DocType: RESIGNATION_LETTER, FNF_SETTLEMENT, RELIEVING_LETTER, EXPERIENCE_LETTER.' },
  ...
  { moduleKey: 'epc', submoduleKey: 'slddrw_source', documentType: 'SLDDRW', displayName: 'SolidWorks Drawing Source File (.slddrw)', rootPrefix: 'epc-slddrw', pathTemplate: 'epc-slddrw/{DrawingControlId}/{Timestamp}-{filename}', revisionMode: 'none', notes: 'epc-slddrw-job-routes.ts — source .slddrw file uploaded by user for extraction job. Governed via resolveGcsPath(). Tokens: DrawingControlId, Timestamp (Date.now()), filename (sanitized). Dual Document Saving Policy applies. 2026-06.' },
  // Vendor Compliance — mandatory onboarding documents for every BP (Business Partner).
  // GCS root: TPEL/VENDORS. Revision mode: numeric (immutable revision chain).
  // Handler: server/vendor-compliance-routes.ts — POST /:vendorId/:docType
  // Path built as: TPEL/VENDORS/{BPCode}/{DocType}/rev-{RevNo}/{Seq}-{Label}.{Ext}
  // Mandatory subset (approval blocked until present): GST_CERTIFICATE, PAN_CARD, CANCELLED_CHEQUE.
  // Max file size: 20 MB. Allowed: PDF, JPG, PNG, JPEG, WEBP, HEIC, TIFF.
  { moduleKey: 'vendor', submoduleKey: 'compliance', documentType: 'GST_CERTIFICATE',        displayName: 'GST Certificate',                rootPrefix: 'TPEL/VENDORS', pathTemplate: 'TPEL/VENDORS/{BPCode}/GST_CERTIFICATE/rev-{RevNo}/{Seq}-gst-certificate.{Ext}',          revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'MANDATORY — vendor approval blocked until present. Immutable revision chain. vendor-compliance-routes.ts. {BPCode}=SAP BP code (e.g. V10001). {RevNo}=zero-padded 2-digit revision (01, 02…). {Seq}=001.' },
  { moduleKey: 'vendor', submoduleKey: 'compliance', documentType: 'PAN_CARD',               displayName: 'PAN Card',                       rootPrefix: 'TPEL/VENDORS', pathTemplate: 'TPEL/VENDORS/{BPCode}/PAN_CARD/rev-{RevNo}/{Seq}-pan-card.{Ext}',                      revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'MANDATORY — vendor approval blocked until present. Immutable revision chain. vendor-compliance-routes.ts.' },
  { moduleKey: 'vendor', submoduleKey: 'compliance', documentType: 'CANCELLED_CHEQUE',       displayName: 'Cancelled Cheque / Bank Proof',  rootPrefix: 'TPEL/VENDORS', pathTemplate: 'TPEL/VENDORS/{BPCode}/CANCELLED_CHEQUE/rev-{RevNo}/{Seq}-cancelled-cheque.{Ext}',     revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'MANDATORY — vendor approval blocked until present. Immutable revision chain. vendor-compliance-routes.ts.' },
  { moduleKey: 'vendor', submoduleKey: 'compliance', documentType: 'MSME_CERTIFICATE',       displayName: 'MSME Certificate',               rootPrefix: 'TPEL/VENDORS', pathTemplate: 'TPEL/VENDORS/{BPCode}/MSME_CERTIFICATE/rev-{RevNo}/{Seq}-msme-certificate.{Ext}',     revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — not required for vendor approval. Immutable revision chain. vendor-compliance-routes.ts.' },
  { moduleKey: 'vendor', submoduleKey: 'compliance', documentType: 'VENDOR_REGISTRATION_FORM', displayName: 'Vendor Registration Form',     rootPrefix: 'TPEL/VENDORS', pathTemplate: 'TPEL/VENDORS/{BPCode}/VENDOR_REGISTRATION_FORM/rev-{RevNo}/{Seq}-vendor-registration-form.{Ext}', revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — not required for vendor approval. Immutable revision chain. vendor-compliance-routes.ts.' },
  { moduleKey: 'vendor', submoduleKey: 'compliance', documentType: 'CONTACT_DETAILS_SHEET',  displayName: 'Contact Details Sheet',          rootPrefix: 'TPEL/VENDORS', pathTemplate: 'TPEL/VENDORS/{BPCode}/CONTACT_DETAILS_SHEET/rev-{RevNo}/{Seq}-contact-details-sheet.{Ext}',  revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — not required for vendor approval. Immutable revision chain. vendor-compliance-routes.ts.' },
  { moduleKey: 'vendor', submoduleKey: 'compliance', documentType: 'ADDRESS_PROOF',           displayName: 'Address Proof',                  rootPrefix: 'TPEL/VENDORS', pathTemplate: 'TPEL/VENDORS/{BPCode}/ADDRESS_PROOF/rev-{RevNo}/{Seq}-address-proof.{Ext}',             revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — not required for vendor approval. Immutable revision chain. vendor-compliance-routes.ts.' },
  // Company Information Master — 9 compliance document types + 3 branding asset types
  // GCS root: TPEL/COMPANY. Revision mode: numeric (immutable revision chain).
  // Handler: server/company-routes.ts — POST /:id/documents/:docType
  // Path: TPEL/COMPANY/{CompanyCode}/{DocType}/rev-{RevNo}/{Seq}-{label}.{Ext}
  // Branding assets: TPEL/COMPANY/{CompanyCode}/BRANDING/{ASSET_TYPE}/{filename}
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_GST_CERTIFICATE',        displayName: 'Company GST Certificate',          rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/GST_CERTIFICATE/rev-{RevNo}/{Seq}-gst-certificate.{Ext}',                revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'MANDATORY — company compliance document. Immutable revision chain. company-routes.ts. {CompanyCode}=company_master.company_code (e.g. TPEL). {RevNo}=zero-padded 2-digit revision.' },
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_PAN_CARD',               displayName: 'Company PAN Card',                 rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/PAN_CARD/rev-{RevNo}/{Seq}-pan-card.{Ext}',                            revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'MANDATORY — company compliance document. Immutable revision chain. company-routes.ts.' },
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_CANCELLED_CHEQUE',       displayName: 'Company Cancelled Cheque',          rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/CANCELLED_CHEQUE/rev-{RevNo}/{Seq}-cancelled-cheque.{Ext}',            revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'MANDATORY — company compliance document. Immutable revision chain. company-routes.ts.' },
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_INCORPORATION_CERTIFICATE', displayName: 'Incorporation Certificate',      rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/INCORPORATION_CERTIFICATE/rev-{RevNo}/{Seq}-incorporation-certificate.{Ext}', revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'MANDATORY — company compliance document. Immutable revision chain. company-routes.ts.' },
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_IEC_CERTIFICATE',        displayName: 'IEC Certificate',                  rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/IEC_CERTIFICATE/rev-{RevNo}/{Seq}-iec-certificate.{Ext}',              revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — company export compliance. Immutable revision chain. company-routes.ts.' },
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_LUT_COPY',               displayName: 'LUT Copy',                         rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/LUT_COPY/rev-{RevNo}/{Seq}-lut-copy.{Ext}',                            revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — Letter of Undertaking for export without GST. Immutable revision chain. company-routes.ts.' },
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_MSME_CERTIFICATE',       displayName: 'MSME Certificate',                 rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/MSME_CERTIFICATE/rev-{RevNo}/{Seq}-msme-certificate.{Ext}',            revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — MSME Udyam registration. Immutable revision chain. company-routes.ts.' },
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_FACTORY_LICENSE',        displayName: 'Factory License',                  rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/FACTORY_LICENSE/rev-{RevNo}/{Seq}-factory-license.{Ext}',              revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — factory / manufacturing license. Immutable revision chain. company-routes.ts.' },
  { moduleKey: 'company', submoduleKey: 'compliance', documentType: 'COMPANY_PF_ESI_DOCUMENT',        displayName: 'PF / ESI Documents',               rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/PF_ESI_DOCUMENT/rev-{RevNo}/{Seq}-pf-esi-document.{Ext}',              revisionMode: 'numeric', maxFileSizeMb: 20, allowedMimeTypes: ['application/pdf','image/jpeg','image/png','image/webp'], notes: 'Optional — PF/ESI registration documents. Immutable revision chain. company-routes.ts.' },
  // Branding assets — image-only, 2 MB max, replace lifecycle
  { moduleKey: 'company', submoduleKey: 'branding',   documentType: 'COMPANY_LOGO',                   displayName: 'Company Logo',                     rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/BRANDING/LOGO/{filename}',                                                revisionMode: 'none',    maxFileSizeMb: 2,  allowedMimeTypes: ['image/jpeg','image/png','image/webp'], notes: 'Company logo image. Image-only (JPEG/PNG/WEBP). Max 2 MB. Replace lifecycle — new upload overwrites stored GCS path in company_master.logo_gcs_path. Orphan cleanup Phase 2. company-routes.ts POST /:id/branding/logo.' },
  { moduleKey: 'company', submoduleKey: 'branding',   documentType: 'COMPANY_SIGNATURE',              displayName: 'Authorized Signatory Image',        rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/BRANDING/SIGNATURE/{filename}',                                           revisionMode: 'none',    maxFileSizeMb: 2,  allowedMimeTypes: ['image/jpeg','image/png','image/webp'], notes: 'Authorized signatory image for PDF letterheads. Image-only. Max 2 MB. Replace lifecycle. company-routes.ts POST /:id/branding/signature.' },
  { moduleKey: 'company', submoduleKey: 'branding',   documentType: 'COMPANY_SEAL',                   displayName: 'Company Seal / Stamp',              rootPrefix: 'TPEL/COMPANY', pathTemplate: 'TPEL/COMPANY/{CompanyCode}/BRANDING/SEAL/{filename}',                                                revisionMode: 'none',    maxFileSizeMb: 2,  allowedMimeTypes: ['image/jpeg','image/png','image/webp'], notes: 'Company seal/stamp image. Image-only. Max 2 MB. Replace lifecycle. company-routes.ts POST /:id/branding/seal.' },
];
