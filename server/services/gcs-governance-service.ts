...
const SEED_RULES = [
  ...
  { moduleKey: 'epc', submoduleKey: 'co',           documentType: 'CO_DOCUMENT',     displayName: 'Customer Order Document',     rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/SOR_{Code}/Sales/Order_Contract/{Seq}-{Label}-rev-{rev}.pdf',  revisionMode: 'numeric',   notes: 'customer-order-document-routes.ts. Revision is filename suffix. {Code}=EPC project code (e.g. 2627-018); folder prefixed SOR_.' },
  ...
  { moduleKey: 'epc', submoduleKey: 'final_offer',   documentType: 'FINAL_OFFER',   displayName: 'Final Locked Offer PDF',             rootPrefix: 'TPEL', pathTemplate: 'TPEL/{CC}/{CO}/{Cust}/{FY}/SOR_{Code}/Sales/Final_Offer/{OfferNo}-rev-{rev}.pdf',  revisionMode: 'numeric',   notes: 'Immutable snapshot auto-saved when an offer is converted to an order (offer status → Order Confirmed). One file per offer revision. Stored under the project Sales folder alongside CO documents. offer-conversion.ts via storeFinalOfferPdfToGcs(). {Code}=EPC project code (e.g. 2627-019). {OfferNo}=offer number with slashes replaced by dashes.' },
  // DVS
