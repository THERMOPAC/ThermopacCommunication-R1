...
import { registerSecurityEnforcementRoutes } from "./admin-security-enforcement-routes";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  registerSecurityEnforcementRoutes(app);
  ...
  */
  ...
  // ── EPC SolidWorks Structuring Agent API ─────────────────────────────────────
  const { default: epcStructureJobRoutes } = await import('./epc-structure-job-routes');
  ...
  // ── Operational Intelligence — Phase 1D CAPA ──────────────────────────────
  const { oiCapaRouter } = await import('./oi-capa-routes');
  app.use('/api/oi', ensureAuthenticated, oiCapaRouter);
  ...
  // ── PLC — Phase 2: RFQ + Evaluation routes ───────────────────────────────
  const { setupPlcRfqRoutes } = await import('./plc-rfq-routes');
  setupPlcRfqRoutes(app);
  ...
  // ── PLC — Phase 4: SAP / Governance / Escalation routes ────────────────────
  ...
  const { runGovernanceParityCheck } = await import('./services/gcs-governance-service');
