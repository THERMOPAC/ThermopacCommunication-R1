...
## 9. Stage C4 — Common Packed-Column Engine, ECP-Type (`server/engines/llx/llx-ecp-engine.ts`, engine `llx-ecp` v1.1.0; Packing Database `server/engine-framework/packing/database.ts`; distributor modules `server/engine-framework/packing/distributors.ts`; single-phase frictional CEL `server/engine-framework/cel/packing-single-phase.ts`)

**PRELIMINARY ECP-TYPE PACKED COLUMN SCREENING — NOT VENDOR RATING AND NOT FOR FABRICATION.** Vendor neutral: the engine consumes only "Vendor Packing Capacity", "Vendor Pressure Drop", "Vendor Packing Performance" through the Packing Database schema — vendor identity is data, not code. The engine CONSUMES packing data; it never owns packing data. Performance data are curves (tabulated / polynomial with stated valid range) or constants with a stated applicability note; interpolation only — extrapolation is refused. HETS is SYSTEM data (value, unit, operating temperature, solvent, feed, packing, source) — never packing data alone. Rate-based placeholders (HTU, NTU, Ka, interfacial area) are reserved architecture, not calculated. Dry pressure drop carries a PRELIMINARY single-phase frictional prediction framework (ECP-009/ECP-010, v1.1.0) — classified "Preliminary Pressure Drop Prediction — Pending RRBO/NMP Validation"; it never substitutes the vendor WET basis (ECP-007). The C3 generic-throughput percentage is neither an input nor reused as ECP utilization.
...
| ECP-007 | Pressure drop (WET only) | Δp/m from the packing record's WET basis only (table interpolation inside range, or polynomial inside stated valid range, or constant with applicability note); Δp_total = Δp/m × H_pack. Extrapolation refused (PRESSURE_DROP_OUT_OF_DATA_RANGE ⇒ Not Calculable). Missing basis ⇒ Not Calculable without blocking anything else. No universal Pa/m is ever invented. Dry Δp preliminary prediction moved to ECP-009 (v1.1.0) | data-gated |
| ECP-009 | Preliminary single-phase frictional (dry-bed analog) pressure drop | Source: M. Duss, "Packing pressure drop prediction at low operating pressure: Is there anything new?", Distillation Topical Conference, AIChE Spring Meeting, San Antonio, Texas, April 2013 (Sulzer Chemtech Ltd.); critical Reynolds numbers: M. Zogg, Diss. Nr. 4886, ETH Zürich, 1972 (cited via Duss). ONLY explicitly published equations: d_h = 4/a (eq. 3); Re = u_s·ρ·d_h/η (eq. 4, superficial-velocity basis); F_v = u_s·√ρ (eq. 5); Δp/Δz = c_f·ρ·u_s²/(2·d_h) = c_f·F_v²/(2·d_h) (eqs. 2/6); f = 16/Re (eq. 1) carried as laminar PIPE reference relation ONLY — never a packing c_f (packing laminar c_f ~10× higher, Zogg). Continuous phase only (dry-bed analog, below loading point). c_f exclusively from a source-tagged packing frictionFactorData basis (constant or curve vs phaseReynoldsNumber, interpolation only) — the paper publishes NO packing c_f(Re) correlation and its tabulated c_f values are vendor-software outputs (excluded by directive); missing ⇒ Not Calculable + NO_PACKING_FRICTION_FACTOR_DATA. Flow regime vs PUBLISHED anchors only: φ = 45° ⇒ Re_crit ≈ 250; φ = 30° ⇒ Re_crit ≈ 450; angle absent and Re < 250 ⇒ Laminar by bounding argument; otherwise Not Determinable — no interpolation over angle | ALWAYS "Preliminary Pressure Drop Prediction — Pending RRBO/NMP Validation" (paper's validated envelope is gas-phase gas/liquid distillation packing — LLX liquid-liquid duty is outside it); definitions (d_h, u_s, Re, F_v) Calculated Screening Result unless underlying data Assumed |
| ECP-010 | Dry pressure-drop verification (paper procedure) | Source-paper quantitative check, steps (a)–(f): back-calculate c_f = 2·(Δp/Δz)·d_h/(ρ·u_s²) from the calculable specific pressure drop (vendor WET basis when present), compute Re (eq. 4), compare against published friction-factor relationships for the tagged corrugation angle; laminar pipe f = 16/Re reported as comparison anchor only. No calculable Δp/Δz ⇒ Not Calculable with explicit reason. Mandatory validated-range statement: LLX operates OUTSIDE the paper's validated range (gas-phase basis) — framework applied as preliminary physics-based analog only | data-gated; Pending Validation when computed |
