...
| DS-SEL-003 | Hydraulic feasibility & diameter selection | U = L_total/C_basis from the frozen sweep row; feasible when U ≤ u_allow; select the SMALLEST practical increment ≥ rounded minimum that is feasible; ungoverned checks (wetting, distributor, recommended loading) skipped-with-notation | Sweep exhaustion ⇒ Not Recommendable, never extrapolated |
| DS-SEL-004 | Capacity-basis hierarchy | Validated entry (Vendor/Pilot, source-referenced) > ECP preliminary screening threshold (upper bound of the Sulzer SMV/SMVP published screening range 35–60 m³/(m²·h), Rauber, Sulzer Chemtech, AIChE 2006; Assumed — Pending Hydraulic Validation). The packing threshold is NOT transferable to ECR; without validated ECR capacity data ECR is Not Recommendable | Active tier always named; no silent substitution; C3 generic % never a substitute |
| DS-SEL-005 | Technology cascade | (1) hydraulic feasibility; (2) direct comparison of actual calculated flooding margins (no tie band); (3) pressure drop only when validated ΔP data exist for all remaining technologies. Residual equivalence ⇒ "Multiple technically acceptable solutions identified. Engineering review required." | No preference is invented; confidence never breaks ties |
...
**V&V registration:** `server/vv/seed-dsel-equation-register.ts` (engine `llx-design-selection`, refs DS-SEL-001…005).
