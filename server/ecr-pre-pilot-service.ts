...
import {
  ...
  ECR_STAGE3_STAGE4_OPTIMIZER_CONFIGURABLE_VERSION,
  ECR_STAGE3_STAGE4_OPTIMIZER_SMALLEST_VERSION,
  ...
  const verified = rows.rows.map((row: any) => {
    ...
    const numericalReplay = (() => {
      switch (row.result?.engine?.version) {
        ...
        case ECR_STAGE3_STAGE4_OPTIMIZER_CONFIGURABLE_VERSION:
          return replayConfigurableStage3Stage4(
            row.processBasis,
            row.processBasis?.stage1SnapshotHash,
            row.result?.controls,
          );
        case ECR_STAGE3_STAGE4_OPTIMIZER_SMALLEST_VERSION:
          return replaySmallestStage3Stage4(
            row.processBasis,
            row.processBasis?.stage1SnapshotHash,
            row.result?.controls,
          );
