...
export function setupProcurementRoutes(app: Router) {
  ...
  app.get('/api/procurement/purchase-orders/preview/:projectId', ensureAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // Step 2: Run a very simple query to avoid column name issues
      // First get all project items
      const projItems = await db.query.projectItems.findMany({
      ...
      // Step 3: For each project item, fetch the master item details
      const itemDetails = [];
      
      for (const projItem of projItems) {
        if (!projItem.itemId) continue;
        
        const masterItem = await db.query.masterItems.findFirst({
          where: eq(masterItems.id, projItem.itemId)
        });
      ...
      console.log(`Found ${itemDetails.length} buy items for project ${project.code}`);
      ...
      // Step 4: Get existing purchase orders for this project
      ...
      if (existingPOIds.length > 0) {
        existingPOItems = await db.query.purchaseOrderItems.findMany({
          where: inArray(purchaseOrderItems.purchaseOrderId, existingPOIds as any[])
        });
      }
      ...
      const availableBuyItems = itemDetails.filter(item => 
      ...
      const vendorsList = vendorIds.length > 0 ? 
        await db.query.vendors.findMany({
          where: inArray(vendors.id, vendorIds as any[])
        }) : [];
