    ...
    try {
      ...
      try {
        ...
        if (sapResult.success) {
          ...
          // Store warning (e.g. address skipped due to duplicate AddressName) in sap_sync_error
          // so the UI can surface it, while keeping sap_sync_status='synced'.
          if (sapResult.warning) {
            await pool.query(
              `UPDATE customers SET sap_sync_status='synced', sap_sync_error=$1, sap_synced_at=NOW(), updated_at=NOW() WHERE id=$2`,
              [sapResult.warning, customerId]
            );
          } else {
            await pool.query(
              `UPDATE customers SET sap_sync_status='synced', sap_sync_error=NULL, sap_synced_at=NOW(), updated_at=NOW() WHERE id=$1`,
        ...
        res.json({ ...updatedCustomer, sapSyncStatus: sapResult.success ? 'synced' : 'failed', sapSyncError: sapResult.error, sapSyncWarning: sapResult.warning });
      } catch (sapError: any) {
  ...
  app.post('/api/customers/:id/retry-sap-sync', ensureAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      if (sapResult.success) {
        ...
        console.log(`✅ [retry-sap-sync] Customer ${customer.bpCode} re-synced to SAP B1 by ${req.user!.username}${sapResult.warning ? ' (with address warning)' : ''}`);
        return res.json({ success: true, warning: sapResult.warning });
      } else {
        await pool.query(
  ...
  app.post('/api/customers/sap-sync', ensureAuthenticated, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (testCardCode) {
        ...
        if (parsedCustRow) {
          console.log(`[customer-sap-sync] TEST PARSED addr — BillLine1="${parsedCustRow.BillAddrLine1}" BillLine2="${parsedCustRow.BillAddrLine2}" BillBlock="${parsedCustRow.BillAddrBlock}" BillBuilding="${parsedCustRow.BillAddrBuilding}" BillCity="${parsedCustRow.BillAddrCity}"`);
          console.log(`[customer-sap-sync] TEST PARSED addr — ShipLine1="${parsedCustRow.ShipAddrLine1}" ShipLine2="${parsedCustRow.ShipAddrLine2}" ShipBlock="${parsedCustRow.ShipAddrBlock}" ShipBuilding="${parsedCustRow.ShipAddrBuilding}" ShipCity="${parsedCustRow.ShipAddrCity}"`);
        }
  ...
  app.post('/api/customers/vendor-sap-sync', ensureAuthenticated, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (testCardCode) {
        ...
        if (parsedRow) {
          console.log(`[vendor-sap-sync] TEST PARSED addr fields — BillLine1="${parsedRow.BillAddrLine1}" BillLine2="${parsedRow.BillAddrLine2}" BillBlock="${parsedRow.BillAddrBlock}" BillBuilding="${parsedRow.BillAddrBuilding}" BillCity="${parsedRow.BillAddrCity}"`);
          console.log(`[vendor-sap-sync] TEST PARSED addr fields — ShipLine1="${parsedRow.ShipAddrLine1}" ShipLine2="${parsedRow.ShipAddrLine2}" ShipBlock="${parsedRow.ShipAddrBlock}" ShipBuilding="${parsedRow.ShipAddrBuilding}" ShipCity="${parsedRow.ShipAddrCity}"`);
        }
