...
/**
 * Special fix for generating inspection orders for Project 2025-4 (ID 6)
 * This handles decimal quantities and special cases for this specific project
 */
export const generateInspectionOrdersForProject6 = async (req: Request, res: Response) => {
  try {
    console.log('== SPECIAL FIX FOR PROJECT 6 (2025-4) - EXECUTION MODE ==');
    const projectId = 6; // Hard-coded project ID
    ...
    // Extract year and project number from project code (e.g., "2025-4")
    const projectCodeParts = project.code.split('-');
    const year = projectCodeParts[0] || '2025';
    const projectNumber = projectCodeParts[1] || '4';
    ...
    return res.status(200).json({
      success: true,
      message: `Created ${createdInspectionOrders.length} inspection orders successfully (SPECIAL FIX for 2025-4)`,
    ...
    console.error('Error in special fix for Project 6:', error);
