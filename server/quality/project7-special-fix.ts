...
/**
 * Special fix for generating inspection orders for Project 2025-5 (ID 7)
 * This handles decimal quantities and special cases for this specific project
 */
export const generateInspectionOrdersForProject7 = async (req: Request, res: Response) => {
  try {
    console.log('== SPECIAL FIX FOR PROJECT 7 (2025-5) - EXECUTION MODE ==');
    const projectId = 7; // Hard-coded project ID
    ...
    // Extract year and project number from project code (e.g., "2025-5")
    const projectCodeParts = project.code.split('-');
    const year = projectCodeParts[0] || '2025';
    const projectNumber = projectCodeParts[1] || '5';
    ...
    return res.status(200).json({
      success: true,
      message: `Created ${createdInspectionOrders.length} inspection orders successfully (SPECIAL FIX for 2025-5)`,
    ...
    console.error('Error in special fix for Project 7:', error);
