...
- **Key Modules & Features**:
    ...
    - **Document Management**: Integrates with GCS for hierarchical storage, metadata tracking, templates, and access control. Includes Drawing Registry for version control, CAD management, and review workflows; and EPC Document Attachment & Retrieval with GCS-backed storage, SHA-256 duplicate detection, contextual download, revision-grouped listing, and audit history. Quotation PDFs use a two-stage TPEL hierarchy: pre-conversion at `TPEL/{Continent}/{Country}/{Customer}/{FY}/Quotations/{Offer}/rev-{NN}/{seq}-{label}.pdf`, post-conversion controlled copy at `TPEL/.../{ ProjectCode}/QTN/{Offer}/rev-na/{seq}-{label}.pdf` with dual checksum verification. Customers table stores `continent_code` and `country_code` (ISO 2-letter) for GCS path construction.
    ...
    - **EPC Control Integrity**: Manages project status cascades, BOM reconciliation, DWG linking enforcement, and automated inspection triggers. Includes stage-gate enforcement for planning records before PO/WO creation.
