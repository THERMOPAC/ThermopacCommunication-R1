...
- **Task Closure & Verification Framework**: Prevents false task closures with a structured verification workflow. Three categories require verification: (1) High/Critical priority tasks, (2) agent-generated tasks, (3) **self-created + self-assigned tasks** — strongest rule: only the assignee's Reporting Manager (or Superuser) can perform final verification/closure. Features include: "Submit for Verification" workflow with evidence collection (6 evidence types), manager/creator verification with approve/reject actions, closure attempt tracking with auto-escalation after 3 failed attempts, verification audit trail via task_history, and a `task_verification_evidence` table. Non-self tasks follow normal verification rules. Status flow: pending → in_progress → pending_verification → completed (verified) or back to in_progress (rejected).
