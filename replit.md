...
- **Dual Document Saving Policy (MANDATORY — baseline v1.0)**: Every file created, uploaded, or system-generated must be saved to GCS (primary) AND mirrored to the Windows file server via Local Document Agent (mirror). This is a mandatory architecture rule enforced on every upload/generation route. Three compliance gates required on every route: **(G1)** call `resolveGcsPath(documentType, tokens)` from `server/utils/gcs-path-resolver.ts` — no hardcoded GCS paths permitted; **(G2)** after GCS success, INSERT `document_agent_jobs` row (`job_type='SAVE_FILE'`, `relative_path`, `expected_sha256`, `source_module`, `source_record_id`); **(G3)** source table must have `mirror_status` (`pending`/`mirrored`/`failed`) and `mirror_job_id` columns updated on agent result. Sequence is inviolable: GCS first → DB record → agent job. GCS failure → abort entirely, no DB record, no job. Mirror failure → document remains valid (GCS authoritative), `mirror_status='failed'`, retryable. GCS never rolled back on mirror failure. No signed URL stored in DB. GCS relative path = agent `local_relative_path` (identical strings). Currently only `company-routes.ts` is fully compliant; 17 modules require remediation (see baseline). Full policy and implementation checklist: `docs/dual-document-saving-policy-baseline-v1.0.md`.
...
- **GCS Path Resolver — single source, no hardcoded paths (2026-06-12)**: All governed GCS upload/save routes must call `resolveGcsPath(documentType, tokens)` from `server/utils/gcs-path-resolver.ts` at write time. This function reads `path_template` from `gcs_governance_rules` (active rule only), substitutes `{Token}` placeholders via `resolvePathTemplate()` from `gcs-governance-service.ts`, and returns the canonical object path. **No route may hardcode a GCS path where a matching governance rule exists.** If no active rule is found for the `document_type`, the upload is rejected with HTTP 503 `GCS_GOVERNANCE_ERROR`. If any `{Token}` placeholder remains unresolved after substitution, the upload is also rejected — no silent fallbacks permitted at any layer. Governed document types currently covered: `COMPANY_LOGO`, `COMPANY_SIGNATURE`, `COMPANY_SEAL`, `COMPANY_{docType}` (compliance docs), `GST_CERTIFICATE` and all vendor compliance types, `CO_DOCUMENT`, `DVS_STAGING`, `WELDER_PHOTO`.
- **MAKE Project Item Engineering Identity Model (ADR v1.0 — approved 2026-07-01)**: Every MAKE Project Item has exactly three canonical identifiers: **(1) Project Item Code** (`project_items.item_code`) — the canonical engineering/business continuity key across the entire EPC/QMS pipeline; **(2) SAP Barcode** (`project_items.code_bars`) — the manufacturing/SAP identity, one stable value per item; **(3) Drawing Number** (`epc_drawing_controls.drawing_number`) — always identical to the SAP Barcode for MAKE items, stable across revisions (only revision code advances). Key rules: `project_item_id` is the internal DB FK (SQL JOINs only). `master_items.item_code` is catalogue-only — never written to any business record. Every downstream business record (BOM, DWG, QPL, PO, WO, IO, Dispatch, Commissioning, Invoice, Documents) shall carry exactly one Project Item Code populated from `project_items.item_code` at INSERT time. One active Drawing per MAKE item (`is_current = true`). One active Inspection Order per MAKE item. Permitted exceptions: virtual WO sub-assembly items (`item_code = null` only); BOM lines store master catalogue code by design; BUY items are out of scope. Future hardening (not yet implemented): UNIQUE constraints on `item_code` and `code_bars`; enforcement of Drawing Number = SAP Barcode; partial unique index on active IOs. Full model and migration scope: `docs/item-code-migration-adr-v1.0.md`.
- **EPC ↔ Quality Management Handover (ADR v1.0 — 2026-06-29)**: The **Inspection Order (IO) is the single handover object** between EPC and Quality Management. Both the WO and the IO are created at project conception — neither creates the other. The handover is a **linking and signalling event only**, not a creation event. Full ADR: `docs/epc-quality-handover-adr-v1.0.md`. Key rules that must never be violated: **(R1) No new IO is created by EPC.** The system finds the pre-existing IO for the same `project_item_id`, writes `inspection_orders.epc_work_order_id = wo.id` (the link), and sets `epc_work_orders.quality_status = 'in_progress'`. If no IO exists for the item, the action is blocked with a clear error — Quality must generate the IO first. **(R2) Quality executes on the IO without any workflow change.** The Quality team uses the existing Inspections page. No changes to IER, MDR, NCR, or Final Dossier. **(R7) Drawing Released for Manufacturing is the formal EPC → Quality Management handover event.** When `epc_drawing_controls.released_for_manufacturing` becomes `true`, the drawing release handler automatically transitions: QPL (`quality_planning_records` WHERE `project_item_id = drawing.project_item_id` AND `status = 'draft'` AND `source_context = 'work_order'`) from `draft` → `in_progress`; and INS (`inspection_execution_records` WHERE `project_item_id = drawing.project_item_id` AND `status = 'scheduled'` AND `source_context = 'work_order'`) from `scheduled` → `in_progress`. Both updates execute within the same DB transaction as the drawing release. Procurement-only releases (`released_for_procurement` only) do NOT trigger this handover. **(R8) Manual QPL and INS activation actions are removed.** "Start Preparation" (QPL `draft → under_preparation`) and "Start Inspection" (INS `scheduled → in_progress`) are replaced entirely by the automatic R7 trigger. The routes `POST /api/quality-plans/:id/start-preparation` and `POST /api/inspection-executions/:id/start`, and their corresponding UI buttons and dialogs, are removed. QPL status vocabulary replaces `under_preparation` with `in_progress` as the active working state. **(R3) Write-back is automatic and system-driven.** When IO `status → 'completed'` and `epc_work_order_id IS NOT NULL`, the system writes `epc_work_orders.quality_status = 'inspection_cleared'`, `quality_cleared_at = NOW()`, `quality_cleared_inspection_id = IO.id`. **(R4) The dispatch gate is a hard block.** `epc_dispatch_readiness` creation is blocked unless `epc_work_orders.quality_status = 'inspection_cleared'`. This gate is already implemented in `server/project-routes.ts` and must not be weakened. **(R5) Quality status vocabulary is fixed.** Values: `pending_inspection` (default) → `in_progress` (Ready for Inspection signalled) → `inspection_cleared` (IO completed) or `inspection_failed` (IER failure path). No other values. **(R6) The existing IER write-back via QPR → `production_exec_id` is a parallel path** for QPR-linked WOs and must remain untouched. The IO-based write-back and the IER-based write-back are additive — both can set `inspection_cleared` on the same WO. The first one to fire wins; the second is a no-op due to the `AND quality_status != 'inspection_cleared'` guard. **Schema**: one column added — `inspection_orders.epc_work_order_id INTEGER NULL REFERENCES epc_work_orders(id) ON DELETE SET NULL`. **Server**: `server/quality/epc-io-handover.ts` exports `linkAndSignalInspectionReady(woId, userId)` and `writeBackWOQualityStatus(inspectionOrderId)`. **Routes**: `GET /api/epc/work-orders/:id/inspection-order` (lookup only; EPC trigger route pending architectural review).
...
- **Dual Document Saving Policy Baseline v1.0 (MANDATORY)**: `docs/dual-document-saving-policy-baseline-v1.0.md`
