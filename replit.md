...
- **EPC Project Naming Governance v1**: Canonical project display name = `{code} — {customer_name} — {short_description}` (em dash U+2014, never hyphen). Three source fields (`short_description`, `customer_name`, `project_display_name`) stored in `projects` table. `project_display_name` is computed server-side by `computeProjectDisplayName()` on every create/update — never accepted raw from client. All UI modules MUST use `getProjectDisplayName(p)` from `client/src/lib/project-utils.ts`; direct string concatenation of `code + name` is prohibited. Legacy records backfilled 2026-05-18 (26 projects). Offer→project conversion also writes all three fields atomically.
