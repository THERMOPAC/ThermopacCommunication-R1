# Payroll TDS Calculation Governance — Correction Plan v1.1
**Date:** 2026-05-07  
**Supersedes:** v1.0 (2026-05-07)  
...
### Step D — Employee TDS Refund / Catch-up
After re-computing, the `tds_monthly_records` for prior months will show the corrected (lower) monthly TDS amounts. The spread-forward mechanism will automatically compute a higher TDS for the remaining months to account for the prior under-deduction (because `tdsDeductedYtd` will now correctly reflect the corrected lower amounts). No manual catch-up entry is needed.
