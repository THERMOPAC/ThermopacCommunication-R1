...
Status: historical inactive candidate and offline evidence. Runtime Nix reduction
has since been authorized here; the active configuration is `replit.nix` and
`scripts/production-run.sh`. This does **not** activate workspace-history
exclusion or imply that the app has been published.
...
`production-nix-candidate.nix` remains an audit snapshot. The active six roots
omit a direct GCC runtime root (the GCC library is verified in Chromium's
closure). The build generates a Python-scoped launcher from evaluated Nix paths.
...
chromium and pdftoppm resolve from the candidate roots. The production run wrapper
places the build-generated shim ahead of module Python on PATH.

### Historical isolated candidate launcher
...
the active build-generated launcher instead of this historical test-only shim.
...
## Historical proposed configuration reconciliation
...
## Historical full-production acceptance gates
