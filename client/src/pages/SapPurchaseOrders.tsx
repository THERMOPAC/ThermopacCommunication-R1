...
function PurchaseOrdersContent() {
    ...
    if (isDerivedClosed) {
      return (
        <span
          ...
          title="PO header is still open in SAP, but no receivable lines remain for GRPO."
        >
  ...
  const handleGrpoSubmit = async () => {
    ...
    if (grpoAttachments.length === 0) {
      toast({ title: 'Attachment Required', description: 'At least one attachment is mandatory before creating GRPO.', variant: 'destructive' });
      setGrpoActiveTab('attachments');
      return;
    }
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <Button 
                        ...
                        onClick={async () => {
                          toast({ title: 'Generating PDF...', description: `Fetching PO-${order.DocNum} details from SAP` });
                          try {
                            const resp = await fetch(`/api/sap/b1/purchase/orders/${order.DocEntry}`);
                            const result = await resp.json();
                            if (!resp.ok || !result.success) {
                              toast({ title: 'PDF Failed', description: result.error || 'Could not fetch PO data', variant: 'destructive' });
                              return;
                            }
                            const po = result.data;
                            const lines = po.DocumentLines || [];
                            const sapStatus = (po.SapHeaderStatus === 'bost_Open' || po.SapHeaderStatus === 'O') ? 'Open' : 'Closed';
                            ...
                            doc.text('THERMOPAC', marginL, 12);
                            doc.setFontSize(10);
                            doc.setFont('helvetica', 'normal');
                            doc.text('Purchase Order', marginL, 19);
                            doc.setFontSize(14);
                            doc.setFont('helvetica', 'bold');
                            doc.text(`PO-${po.DocNum}`, pageW - marginR, 12, { align: 'right' });
                            doc.setFontSize(9);
                            doc.setFont('helvetica', 'normal');
                            doc.text(`Status: ${sapStatus}`, pageW - marginR, 19, { align: 'right' });
                            y = 35;

                            doc.setTextColor(0, 0, 0);
                            doc.setFontSize(9);
                            const infoCol1 = marginL;
                            const infoCol2 = pageW / 2 + 5;

                            const addField = (label: string, value: string, x: number, row: number) => {
                              const fy = y + row * 6;
                              doc.setFont('helvetica', 'bold');
                              doc.text(label + ':', x, fy);
                              doc.setFont('helvetica', 'normal');
                              doc.text(value, x + doc.getTextWidth(label + ':  '), fy);
                            };
                            ...
                            addField('Ref No.', po.NumAtCard || 'N/A', infoCol2, 3);
                            y += 30;
                            ...
                            const cols = [
                              { header: '#', w: 8 },
                              { header: 'Item Code', w: 28 },
                              { header: 'Description', w: 58 },
                              { header: 'Qty', w: 16 },
                              { header: 'Open Qty', w: 18 },
                              { header: 'Unit Price', w: 22 },
                              { header: 'Total', w: 24 },
                              { header: 'Status', w: 14 },
                            ];
                            ...
                            doc.setFontSize(7.5);
                            let cx = marginL + 1;
                            cols.forEach(c => { doc.text(c.header, cx, y); cx += c.w; });
                            y += 5;
                            ...
                            lines.forEach((line: any, idx: number) => {
                              ...
                              const lineStatus = (line.LineStatus === 'bost_Close' || line.LineStatus === 'C') ? 'Closed' : 'Open';
                              const openQty = parseFloat(line.RemainingOpenQuantity ?? line.OpenQuantity ?? 0);
                              cx = marginL + 1;
                              doc.text(String(idx + 1), cx, y); cx += cols[0].w;
                              doc.text(String(line.ItemCode || '-'), cx, y); cx += cols[1].w;
                              const desc = String(line.ItemDescription || line.Dscription || line.FreeTxt || '-');
                              doc.text(desc.length > 42 ? desc.substring(0, 40) + '..' : desc, cx, y); cx += cols[2].w;
                              doc.text(String(parseFloat(line.Quantity || 0).toFixed(2)), cx, y); cx += cols[3].w;
                              doc.text(String(openQty.toFixed(2)), cx, y); cx += cols[4].w;
                              doc.text(formatCurrency(line.UnitPrice || line.Price || 0), cx, y); cx += cols[5].w;
                              doc.text(formatCurrency(line.LineTotal || 0), cx, y); cx += cols[6].w;
                              doc.text(lineStatus, cx, y);
                              y += 5;
                            });
                            ...
                            doc.setFontSize(9);
                            const totX = pageW - marginR - 60;
                            doc.setFont('helvetica', 'normal');
                            doc.text('Subtotal:', totX, y);
                            doc.text(formatCurrency((po.DocTotal || 0) - (po.VatSum || 0)), totX + 35, y, { align: 'right' });
                            y += 5;
                            doc.text('Tax (VAT):', totX, y);
                            doc.text(formatCurrency(po.VatSum || 0), totX + 35, y, { align: 'right' });
                            y += 5;
                            doc.setFont('helvetica', 'bold');
                            doc.text('Total:', totX, y);
                            doc.text(formatCurrency(po.DocTotal || 0), totX + 35, y, { align: 'right' });
                            y += 8;
                            ...
                            if (po.Comments) {
                              ...
                              doc.setFont('helvetica', 'normal');
                              const commentLines = doc.splitTextToSize(po.Comments, contentW);
                              doc.text(commentLines, marginL, y + 4);
                            }

                            doc.setFontSize(7);
                            doc.setTextColor(150, 150, 150);
                            doc.text(`Generated: ${new Date().toLocaleString('en-IN')} | THERMOPAC QMS`, marginL, doc.internal.pageSize.getHeight() - 8);

                            doc.save(`PO-${po.DocNum}.pdf`);
                            toast({ title: 'PDF Downloaded', description: `PO-${po.DocNum} saved successfully` });
                          } catch (err: any) {
                            toast({ title: 'PDF Failed', description: err.message || 'Error generating PDF', variant: 'destructive' });
                          }
                        }}
                      >
        ...
        <DialogContent className="max-w-[98vw] w-[1400px] max-h-[96vh] h-[96vh] overflow-hidden p-0 gap-0 flex flex-col">
          ...
          {poDetailLoading ? (
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              ...
              {(poDetail?.canCreateGRPO ?? selectedOrder?.canCreateGRPO) === true && (
              ...
              <div className="bg-gray-100 border-t px-4 py-0.5 flex items-center justify-between text-[12px] text-gray-500 shrink-0">
                ...
                <span className="flex items-center gap-3">
                  <span>SAP Header: <span className="font-semibold">{(poDetail?.SapHeaderStatus || selectedOrder?.SapHeaderStatus) === 'bost_Open' || (poDetail?.SapHeaderStatus || selectedOrder?.SapHeaderStatus) === 'O' ? 'Open' : 'Closed'}</span></span>
          ...
          <Tabs value={grpoActiveTab} onValueChange={setGrpoActiveTab} className="flex-1 flex flex-col min-h-0">
            <TabsList className="shrink-0 w-full justify-start">
              ...
              <TabsTrigger value="attachments" className="text-xs gap-1.5 font-bold">
                ...
                {grpoAttachments.length === 0 ? (
                  <Badge variant="destructive" className="ml-1 h-4 px-1 text-[10px]">Required</Badge>
                ) : (
                  <Badge className="ml-1 h-4 px-1 text-[10px] bg-green-600">✓</Badge>
                )}
              </TabsTrigger>
            ...
            <TabsContent value="attachments" className="flex-1 overflow-auto mt-2">
              <div className="space-y-4">
                ...
                {grpoAttachments.length === 0 && (
                  <div className="text-center py-8 border-2 border-dashed border-red-200 rounded-lg bg-red-50/50">
                    <Paperclip className="h-10 w-10 mx-auto mb-2 text-red-300" />
                    <p className="text-sm font-medium text-red-600">At least one attachment is required</p>
                    <p className="text-xs mt-1 text-gray-500">Upload delivery challans, test certificates, or invoices before creating GRPO</p>
                  </div>
                )}
              </div>
            </TabsContent>
