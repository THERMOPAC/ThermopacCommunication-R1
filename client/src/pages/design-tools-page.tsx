...
function HelicalCoilPressureLossCalculator() {
  ...
  const generateHelicalCoilPDF = async () => {
    if (!result) return;
    ...
    doc.setFontSize(18);
    doc.setFont('helvetica', 'bold');
    doc.text('HELICAL COIL PRESSURE LOSS CALCULATION', 105, 40, { align: 'center' });
    ...
    doc.setFont('helvetica', 'normal');
    const inputs = [
      `Inner Tube Diameter: ${innerDiameter} mm`,
      `Coil Diameter: ${coilDiameter} mm`,
      `Coil Pitch: ${coilPitch} mm`,
      `Number of Turns: ${numberOfTurns}`,
      `Flow Rate: ${flowRate} m³/hr`,
      `Fluid Temperature: ${temperature} °C`,
      `Fluid: ${fluidPresets[fluidPreset as keyof typeof fluidPresets].name}`,
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('CALCULATION RESULTS', 20, 180);
    
    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    const results = [
      `Tube Length: ${result.tubeLength.toFixed(2)} m`,
      `Flow Velocity: ${result.velocity.toFixed(2)} m/s`,
      `Reynolds Number: ${result.reynolds.toFixed(0)}`,
      `Flow Regime: ${result.flowRegime}`,
      `Dean Number: ${result.deanNumber.toFixed(1)}`,
      `Pressure Drop: ${result.pressureDrop.toFixed(4)} bar`
    ];
    
    results.forEach((resultText, index) => {
      doc.text(resultText, 25, 193 + (index * 10));
    });
    
    // Footer
    doc.setFontSize(10);
    doc.setFont('helvetica', 'italic');
    doc.text('Calculation includes Dean number correction for helical coil curvature effects', 105, 260, { align: 'center' });
    ...
    // Save the PDF
    doc.save('thermopac-helical-coil-pressure-loss-calculation.pdf');
  };
      ...
      {result !== null && (
        <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex justify-between items-center mb-3">
            ...
            <Button 
              onClick={generateHelicalCoilPDF}
              variant="outline" 
              size="sm"
...
function CombustionChamberPressureLossCalculator() {
  ...
  const generateCombustionChamberPDF = async () => {
    if (!result) return;
    ...
    doc.setFontSize(18);
    doc.setFont('helvetica', 'bold');
    doc.text('COMBUSTION CHAMBER PRESSURE LOSS CALCULATION', 105, 40, { align: 'center' });
    ...
    doc.setFont('helvetica', 'normal');
    const inputs = [
      `Inlet Pressure: ${inletPressure} ${pressureUnit}`,
      `Inlet Temperature: ${inletTemperature} °C`,
      `Mass Flow Rate: ${massFlowRate} kg/s`,
      `Chamber Length: ${chamberLength} m`,
      `Chamber Diameter: ${chamberDiameter} m`,
      `Surface Roughness: ${surfaceRoughness || 'Smooth'} mm`,
      `Gas Density: ${density} kg/m³`,
      `Dynamic Viscosity: ${viscosity} Pa·s`,
      `Heat Capacity Ratio: ${heatCapacityRatio}`,
      `K-factor for Local Losses: ${kFactor}`
    ];
    
    inputs.forEach((input, index) => {
      doc.text(input, 25, 78 + (index * 9));
    });
    ...
    const results = [
      ...
      `Reynolds Number: ${result.reynolds.toFixed(0)}`,
      `Flow Regime: ${result.flowRegime}`,
      `Friction Factor: ${result.frictionFactor.toFixed(6)}`,
      `Friction Loss: ${result.frictionLoss.toFixed(2)} mbar`,
      `Local Losses: ${result.localLoss.toFixed(2)} mbar`,
      `Total Pressure Loss: ${result.totalLoss.toFixed(2)} mbar`
    ];
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'italic');
    doc.text('Calculation based on Darcy-Weisbach equation with Swamee-Jain friction factor', 105, 260, { align: 'center' });
    ...
    // Save the PDF
    doc.save('thermopac-combustion-chamber-pressure-loss-calculation.pdf');
  };
      ...
      {result !== null && (
        <div className="mt-4 p-4 bg-orange-50 border border-orange-200 rounded-lg">
          <div className="flex justify-between items-center mb-3">
            ...
            <Button 
              onClick={generateCombustionChamberPDF}
              variant="outline" 
              size="sm"
...
function ExpansionTankCapacityCalculator() {
  ...
  const generateExpansionTankPDF = async () => {
    if (!result) return;
    ...
    doc.setFontSize(18);
    doc.setFont('helvetica', 'bold');
    doc.text('EXPANSION TANK CAPACITY CALCULATION', 105, 40, { align: 'center' });
    ...
    doc.setFont('helvetica', 'normal');
    const inputs = [
      `Total Fluid Volume: ${totalVolume} ${volumeUnit}`,
      `Fluid Type: ${fluidPresets[fluidPreset as keyof typeof fluidPresets].name}`,
      `Minimum Temperature: ${minTemperature} °C`,
      `Maximum Temperature: ${maxTemperature} °C`,
      `Thermal Expansion Coefficient: ${expansionCoefficient} /°C`,
      `Safety Margin: ${safetyMargin}%`,
      `Pre-charge Pressure: ${preChargePresssure || 'Not specified'} bar`,
      `Operating Pressure: ${operatingPressure || 'Not specified'} bar`
    ];
    ...
    const results = [
      ...
      `Fluid Expansion Volume: ${result.expansionVolume.toFixed(2)} liters`,
      `Standard Tank Size: ${result.tankSize.toFixed(2)} liters`,
      `Tank Type: ${result.tankType}`,
      `Pressurized Tank Size: ${result.pressurizedTankSize.toFixed(2)} liters`,
      `Recommended Capacity: ${Math.max(result.tankSize, result.pressurizedTankSize).toFixed(2)} liters`
    ];
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('DESIGN NOTES', 20, 240);
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    const notes = [
      '• Tank should accommodate thermal expansion with safety margin',
      `• ${result.tankType === "Open/Atmospheric Tank" 
          ? "Open tank suitable for low-pressure systems" 
          : "Pressurized tank recommended for higher system pressures"}`,
      '• Consider installation of overflow and make-up connections',
      '• Insulation may be required to minimize heat loss'
    ];
    
    notes.forEach((note, index) => {
      doc.text(note, 25, 253 + (index * 7));
    });
    
    // Save the PDF
    doc.save('thermopac-expansion-tank-capacity-calculation.pdf');
  };
      ...
      {result !== null && (
        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex justify-between items-center mb-3">
            ...
            <Button 
              onClick={generateExpansionTankPDF}
              variant="outline" 
              size="sm"
...
function ThermalOilPumpSizingCalculator() {
  ...
  const generatePDF = async () => {
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('CALCULATION RESULTS', 20, 150);
    ...
    results.forEach((resultText, index) => {
      doc.text(resultText, 25, 163 + (index * 10));
    });
    
    // Engineering Analysis
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('ENGINEERING ANALYSIS', 20, 220);
    ...
    analysis.forEach((item, index) => {
      doc.text(item, 25, 233 + (index * 10));
    });
    
    // Add page break before detailed breakdown
    doc.addPage();
          ...
          {isGasMode && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-sm font-medium">Z factor</label>
                  <Input value={zFactor} onChange={e => setZFactor(e.target.value)} placeholder="1.0" />
                  <p className="text-xs text-muted-foreground mt-1">Ideal gas = 1.0</p>
                </div>
              </div>
            </div>
          )}
