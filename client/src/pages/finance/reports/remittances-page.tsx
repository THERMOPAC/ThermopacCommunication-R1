...
export default function RemittancesReportPage() {
  ...
  // Query for remittances report data
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['/api/finance/reports/remittances', dateRange, selectedCurrency],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/finance/reports/remittances?startDate=${startDate}&endDate=${endDate}${currencyParam}`);
      if (!response.ok) {
        throw new Error('Failed to fetch remittances report');
      }
    ...
    return (
      <Layout>
        <div className="flex items-center justify-center min-h-screen">
          ...
          <p>Loading Remittances Report...</p>
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      <Helmet>
        <title>Remittances Report | Thermopac Finance</title>
      </Helmet>
      
      <div className="container mx-auto py-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold pl-4">Remittances Report</h1>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Filter the remittances report by date range and currency
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Remittances Summary</CardTitle>
            <CardDescription>
            ...
            {data ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <Card>
                    <CardHeader className="py-4">
                      <CardTitle className="text-base">Total Remittances</CardTitle>
                    </CardHeader>
                    <CardContent>
                      {selectedCurrency === 'USD' || selectedCurrency === 'all' ? (
                        <p className="text-2xl font-bold text-right">
                          {formatUSD(data.totalRemittances || 0)}
                        </p>
                      ) : (
                        <p className="text-2xl font-bold text-right">
                          {formatRupees(data.totalRemittances || 0)}
                        </p>
                      )}
                      {selectedCurrency === 'all' && (
                        <p className="text-sm text-muted-foreground text-right mt-1">
                          ~ {formatRupees(data.totalRemittancesINR || data.totalRemittances * 85.55 || 0, true)}
                        </p>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardHeader className="py-4">
                      <CardTitle className="text-base">Total BRCs Issued</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-2xl font-bold text-right">
                        {data.totalBRCs || 0}
                  ...
                  <Card>
                    <CardHeader className="py-4">
                      <CardTitle className="text-base">Pending BRCs</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-2xl font-bold text-right">
                        {data.pendingBRCs || 0}
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>BRC Status</TableHead>
                      <TableHead>BRC Document</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {data.remittances && data.remittances.length > 0 ? (
                      data.remittances.map((remittance: any, index: number) => (
                        <TableRow key={index}>
                          <TableCell>{remittance.remittanceNumber}</TableCell>
                          <TableCell>{formatDate(remittance.date)}</TableCell>
                          <TableCell>{remittance.customerName}</TableCell>
                          <TableCell>{remittance.invoiceNumber}</TableCell>
                          <TableCell>
                            {selectedCurrency === 'USD' || selectedCurrency === 'all' 
                              ? formatUSD(remittance.amount || 0)
                              : formatRupees(remittance.amount || 0)}
                          </TableCell>
                          <TableCell>
                            <span className={`px-2 py-1 rounded-full text-xs ${
                              remittance.brcStatus === 'Issued' 
                                ? 'bg-green-100 text-green-800' 
                                : remittance.brcStatus === 'Pending' 
                                  ? 'bg-yellow-100 text-yellow-800' 
                                  : 'bg-gray-100 text-gray-800'
                            }`}>
                              {remittance.brcStatus}
                            </span>
                          </TableCell>
                          <TableCell>
                            {remittance.brcDocumentUrl ? (
                              <Button 
                                variant="outline" 
                                size="sm"
                                onClick={() => window.open(remittance.brcDocumentUrl, '_blank')}
                              >
                                View
                              </Button>
                            ) : (
                      ...
                      <TableRow>
                        <TableCell colSpan={7} className="text-center py-8">
                          No remittances found for the selected period.
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
              ...
              <div className="text-center py-8">
                <p className="text-muted-foreground">
                  Select a date range and currency to view the remittances report.
                </p>
              </div>
            )}
          </CardContent>
