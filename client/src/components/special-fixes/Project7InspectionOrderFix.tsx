...
/**
 * Special component to fix inspection order generation for Project 2025-5 (ID 7)
 */
const Project7InspectionOrderFix = () => {
  const generateMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/quality/inspection-orders/special-fix-project-7", {
        method: "POST",
        headers: {
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/quality/inspection-orders/project/7"] });
    },
    onError: (error) => {
      toast({
        title: "Error",
  ...
  const handleGenerateClick = () => {
    if (confirm("Are you sure you want to generate inspection orders for Project 2025-5 (ID 7) using the special fix?")) {
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto my-4">
      <CardHeader>
        <CardTitle>Project 2025-5 Special Fix</CardTitle>
        <CardDescription>
          This is a special tool to fix inspection order generation for Project 2025-5 (ID 7)
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Alert className="mb-4">
          ...
          <AlertDescription>
            This tool will attempt to generate inspection orders for Project 2025-5 (ID 7) 
...
export default Project7InspectionOrderFix;