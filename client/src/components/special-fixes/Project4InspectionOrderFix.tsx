...
/**
 * Special component to fix inspection order generation for Project 2025-2 (ID 4)
 */
const Project4InspectionOrderFix = () => {
  const generateMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/quality/inspection-orders/special-fix-project-4", {
        method: "POST",
        headers: {
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/quality/inspection-orders/project/4"] });
    },
    onError: (error) => {
      toast({
        title: "Error",
  ...
  const handleGenerateClick = () => {
    if (confirm("Are you sure you want to generate inspection orders for Project 2025-2 (ID 4) using the special fix?")) {
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto my-4">
      <CardHeader>
        <CardTitle>Project 2025-2 Special Fix</CardTitle>
        <CardDescription>
          This is a special tool to fix inspection order generation for Project 2025-2 (ID 4)
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Alert className="mb-4">
          ...
          <AlertDescription>
            This tool will attempt to generate inspection orders for Project 2025-2 (ID 4) 
...
export default Project4InspectionOrderFix;