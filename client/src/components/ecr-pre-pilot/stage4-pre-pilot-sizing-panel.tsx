...
export default function Stage4PrePilotSizingPanel({ designId }: Props) {
  ...
  return <section data-testid="stage4-pre-pilot-sizing" className="overflow-hidden rounded-md border-2 border-cyan-800 bg-white">
    ...
    {result && <div className="space-y-3 p-3">
      ...
      <div className="rounded border border-amber-300 bg-amber-50 p-3 text-[10px] text-amber-950"><p className="font-semibold">Overall-efficiency dependency: {text(efficiency.status)}</p><p className="mt-1">{text(efficiency.dependency)}</p><p className="mt-1">The output remains blank until a supported mass-transfer/axial-mixing closure calculates efficiency; no manual efficiency or HETS entry is available.</p></div>
