...
- **Concurrency rule:** engines must NEVER mutate the shared EPD registries during a run (no register→calculate→unregister). Use `createPropertyContext([...])` and pass the context to `getProperty`/`getInterfacialTension`; lookup order is context → library → global project registry. User mandated this after Stage C2 review.
...
- **C3 slip-model rule:** generic hydraulic screening uses u_slip = u_K·(1−φ)^n with BOTH u_K and n engineer-entered source-tagged; n = 1 accepted only as explicit Assumed (ASSUMED_HINDRANCE_EXPONENT); rigid-sphere u_t reused as u_K only via explicit option → Pending Validation. Screening band and holdup bounds are configurable criteria, never universal rules; sweeps classify diameters, never recommend one.
