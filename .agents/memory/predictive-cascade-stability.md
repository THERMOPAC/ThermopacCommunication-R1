...
**How to apply:** Keep thermodynamic artifacts, convergence and balance tolerances, and saved inputs frozen. Persist each completed stage with a content hash and require a database-commit ACK before beginning the next stage. Duplicate replay must hash-match the stored checkpoint, and a final result must match the acknowledged sequence.