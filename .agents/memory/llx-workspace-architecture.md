...
POST /revisions/:id/inputs REPLACES the section's data JSON — a partial write (e.g. {design_code} only) WIPES the rest of the section (nozzle_rows, supports...). UPDATE (Aug 2026): upsertInput now does atomic field-level merge in SQL (jsonb || / - on the stored row): incoming keys overwrite, absent keys are PRESERVED, explicit null tombstones delete. Deletion-by-omission no longer works anywhere — callers that clear fields (e.g. preliminary-defaults clear) must send null tombstones. Regression test: tests/stage9-section-merge.test.ts. Recovery source if a section is ever wiped: calculation_runs.input_snapshot (mapped engine inputs) + POST /nozzles/generate (deterministic from master data). design_code (Stage 9, engineer-entered, no default) maps to engine designCode; NOT_ASSIGNED when blank.
