...
def compare(current,previous,gc,gd,points,np):
    return {
...
def record_completed_level(levels,answer,nodes,elapsed,previous,gc,gd,points,np,write):
    """Persist the solved state before fallible refinement post-processing."""
    row=serialize(answer)
    row.update({"nodes":nodes,"elapsedSeconds":elapsed})
    ...
    if previous is not None:
        row.update(compare(answer,previous,gc,gd,points,np))
        write()
    return row
    ...
    try:
        ...
        def branch(branch_row,ic,id_,flux=None,pc=None,pd=None):
            ...
            for nodes in (33,65,129):
                ...
                record_completed_level(levels,answer,nodes,time.monotonic()-t,
                                       previous,gc,gd,common,np,write)
                previous=answer
